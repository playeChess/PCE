...
public:
  ...
  std::string show() {
    ...
    return std::string(1, files[start_square_y]) +
           std::to_string(start_square_x + 1) + " -> " +
           std::string(1, files[end_square_y]) +
           std::to_string(end_square_x + 1);
  }
  ...
  bool am_in(std::vector<Move> moves) {
    for (int i = 0; i < moves.size(); i++) {
      if (moves[i].get_end_coords()[0] == this->get_end_coords()[0] &&
          moves[i].get_end_coords()[1] == this->get_end_coords()[1])
        return true;
    }
    return false;
  }
...
public:
   ...
   */
  virtual bool validation_function(std::array<std::array<Piece *, 8>, 8> board,
   ...
   */
  bool validate_validation(std::array<std::array<Piece *, 8>, 8> board,
                           int x_final, int y_final) {
   ...
   */
  bool check_path(int x_final, int y_final,
...
public:
   ...
   * @return If the move is valid (bool)
   */
  bool validation_function(std::array<std::array<Piece *, 8>, 8> board,
                           int x_final, int y_final) {
    // TODO Add pawn takes
    int x_diff = x_final - this->coords[0];
    int y_diff = y_final - this->coords[1];
    ...
    if (y_diff == 0 && x_diff != 0) {
      if (this->is_white) {
        if (x_diff == 1) {
          ...
          if (board[x_final][y_final] == nullptr &&
              board[x_final][y_final - 1] == nullptr)
            return true;
        }
      } else {
        if (x_diff == -1) {
...
public:
   ...
   * @return The moves (std::vector<PlayeChessEngine::Move>)
   */
  std::vector<PlayeChessEngine::Move>
  get_all_moves(std::array<std::array<pieces::Piece *, 8>, 8> brd, bool white,
                bool from_premove = false) {
    std::vector<PlayeChessEngine::Move> moves;
    for (int i = 0; i < 8; i++) {
      for (int j = 0; j < 8; j++) {
  ...
  bool is_check(bool white) {
    for (auto move : this->get_all_moves(this->board, !white, true)) {
      if (this->board[move.get_end_coords()[0]][move.get_end_coords()[1]] ==
          nullptr)
        continue;
      if (this->board[move.get_end_coords()[0]][move.get_end_coords()[1]]
              ->get_type() == pieces::piece_type::k)
        return true;
    }
    return false;
  }
   ...
   */
  std::array<std::array<pieces::Piece *, 8>, 8>
  transfer(std::array<std::array<pieces::Piece *, 8>, 8> board, int start_x,
  ...
  bool premove_check(PlayeChessEngine::Move move, bool white) {
    if (this->board[move.get_start_coords()[0]][move.get_start_coords()[1]] ==
        nullptr)
      throw std::invalid_argument("No piece at start coords");
    std::array<std::array<pieces::Piece *, 8>, 8> backup = this->board;
    this->board = this->transfer(
        this->board, move.get_start_coords()[0], move.get_start_coords()[1],
        move.get_end_coords()[0], move.get_end_coords()[1]);
  ...
  bool move(PlayeChessEngine::Move move, bool white) {
    if (this->board[move.get_start_coords()[0]][move.get_start_coords()[1]] ==
        ...
        this->board[move.get_start_coords()[0]][move.get_start_coords()[1]]
                ->is_white != white)
      return false;
    if (move.am_in(this->get_moves(move.get_start_coords()[0],
                                   move.get_start_coords()[1]))) {
      this->board = this->transfer(
          this->board, move.get_start_coords()[0], move.get_start_coords()[1],
          move.get_end_coords()[0], move.get_end_coords()[1]);
      this->board[move.get_end_coords()[0]][move.get_end_coords()[1]]
          ->update_coords(move.get_end_coords()[0], move.get_end_coords()[1]);
      return true;
    }
    return false;
  }
  ...
  bool can_castle_row(int row, bool kingside) {
    if (kingside) {
      if (this->board[row][4] != nullptr && this->board[row][7] != nullptr &&
          this->board[row][5] == nullptr && this->board[row][6] == nullptr) {
        if (this->board[row][4]->get_type() == pieces::piece_type::k &&
            this->board[row][7]->get_type() == pieces::piece_type::r &&
            !this->board[row][4]->has_moved &&
            !this->board[row][7]->has_moved) {
          // TODO Add check for check and that the king does not pass through a
    ...
    if (this->board[row][4] != nullptr && this->board[row][0] != nullptr &&
        this->board[row][1] == nullptr && this->board[row][2] == nullptr &&
        this->board[row][3] == nullptr) {
      if (this->board[row][4]->get_type() == pieces::piece_type::k &&
          this->board[row][0]->get_type() == pieces::piece_type::r &&
          !this->board[row][4]->has_moved && !this->board[row][0]->has_moved) {
        // TODO Add check for check and that the king does not pass through a
        // square that is attacked by an enemy piece
        if (!this->is_check(this->board[row][4]->is_white))
          return true;
...
private:
  ...
  PlayeChessEngine::board::Board board =
      PlayeChessEngine::board::Board("r3k2r/8/8/8/8/8/8/R3K2R");
...
public:
  ...
  bool move(bool white) {
    this->clear_screen();
    std::vector<Move> moves =
        this->board.get_all_moves(this->board.get_board(), white);
    for (auto move : moves) {
    ...
    while (!valid) {
      do {
        std::cin >> move;
      } while (move.length() != 4 && move != "exit" && move != "O-O" &&
               move != "O-O-O");
      if (move == "exit")
        return true;
      if (move == "O-O") {
      ...
      Move move_obj =
          Move(start_coords[0], start_coords[1], end_coords[0], end_coords[1]);
